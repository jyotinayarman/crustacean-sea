...
def create_glb_with_pbr_materials(obj_path, textures_dict, output_path):
    ...
    temp_glb = os.path.join(out_dir, "_temp_pbr_export.glb")
    mr_combined_path = os.path.join(out_dir, "_temp_mr_combined.png")
    try:
        mesh = trimesh.load(obj_path)
        mesh.export(temp_glb)
        gltf = pygltflib.GLTF2().load(temp_glb)
    except Exception as e:
        if os.path.isfile(temp_glb):
            try:
                os.remove(temp_glb)
            except OSError:
                pass
    ...
    for p in (temp_glb, mr_combined_path):
