    ...
    def from_single_file(
        ...
        **kwargs,
        ...
        if use_safetensors:
            ...
            # Load model component incrementally
            model_sd = {}
            with safe_open(ckpt_path, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("model."):
                        model_sd[key[6:]] = f.get_tensor(key)
            ...
            # Load vae component incrementally
            vae_sd = {}
            with safe_open(ckpt_path, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("vae."):
                        vae_sd[key[4:]] = f.get_tensor(key)
            ...
            # Load conditioner component incrementally
            conditioner_sd = {}
            with safe_open(ckpt_path, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("conditioner."):
                        conditioner_sd[key[12:]] = f.get_tensor(key)
            conditioner = instantiate_from_config(config['conditioner'])
            if conditioner_sd:
                conditioner.load_state_dict(conditioner_sd)
                del conditioner_sd
                gc.collect()
            ...
            conditioner_sd = ckpt.pop('conditioner', None)
            del ckpt
            gc.collect()
            conditioner = instantiate_from_config(config['conditioner'])
            if conditioner_sd is not None:
                conditioner.load_state_dict(conditioner_sd)
                del conditioner_sd
                gc.collect()
            conditioner.to(load_device)
